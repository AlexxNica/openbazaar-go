...
func newRandomListing() *pb.Listing {
	...
	// skus := make([]*pb.Listing_Item_Sku, len())
	for i := 0; i < len(options); i++ {
		...
		// Set variant and sku values
		// skus := make([]*pb.Listing_Item_Sku, len())
		for j := 0; j < len(options[i].Variants); j++ {
			// Get fake name <= 40 characters
			options[i].Variants[j] = fake.Word()
			if len(options[i].Variants[j]) > 40 {
				options[i].Variants[j] = options[i].Variants[j][:40]
			}
